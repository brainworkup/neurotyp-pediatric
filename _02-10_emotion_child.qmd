...
# Flatten the text
bwu::cat_neuropsych_results(data = data_emotion_self, file = "_02-09_emotion_child_text_self.qmd")
```
...
grp_self <- list(
  t_score = c("BASC-3 SRP ", "PAI", "MMPI-3")
)

# Clean scale names
data_emotion_self <-
  dplyr::mutate(data_emotion_self,
...
# Clean scale names
data_emotion_parent <-
  dplyr::mutate(data_emotion_parent,
    scale = stringr::str_remove_all(
      scale, "PAI |BASC-3 |MMPI-3 "
    )
  )
...
data_emotion_parent <-
  data_emotion |>
  dplyr::filter(test == "basc3_prs_child" | test == "basc3_prs_adolescent" | test == "basc3_prs_college") |>
  dplyr::distinct(.keep_all = FALSE)

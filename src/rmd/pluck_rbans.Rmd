...
saveRDS(input_file_path, "rbans.rds")
...
```{r}
library(tidyverse)
input_file_path <- "data/004004907_3_10_2024Biggie G 12 yo Follow-up ADHD NLD LD eval_scores.csv"
...
process_and_save_rbans_data <- function(rbans_raw,
  ...
  df$percentile[df$scale == "RBANS Update Form A Line Orientation"] <- 37
  df$percentile[df$scale == "RBANS Update Form A Picture Naming"] <- 37
  df$percentile[df$scale == "RBANS Update Form A List Recall"] <- 76
  df$percentile[df$scale == "RBANS Update Form A List Recognition"] <- 63
...
# Write the combined data to a CSV file
output_file_path <- "data/csv/rbans.csv"
